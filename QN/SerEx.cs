...
namespace QN
{
    ...
    public static class SerEx
    {
        ...
        /// <summary>
        ///     When verifying cycles, check for reference to parent owner of instance only. This reduces memory consumption -
        ///     however does not check
        ...
        public static string ToXml<T>(this T item, bool minimal = true, bool removeNamespace = true,
            bool newLineEntitize = true, bool scanXsiDuplicates = true)
        {
        ...
        static void doScanXsiDuplicates<T>(T item)
        {
            __doScanXsiDupInner(item, typeof(T), 0, new HashSet<object>());
        }
        ...
        static void __doScanXsiDupInner(object item, Type t, int depth, HashSet<object> dupl)
        {
            ...
            foreach (var p in t.GetProperties(BindingFlags.Instance | BindingFlags.Public)) {
                if (p.IsSpecialName || !p.PropertyType.IsClass || p.hasAttr<XmlIgnoreAttribute>() || !p.CanRead ||
                    !p.CanWrite || p.Name == "Item") continue;
                var v = p.GetValue(item);
                __doScanXsiDupInner(v, p.PropertyType, depth + 1, dupl);
            }

            var anyAttrFld =
                t.GetField("AnyAttributes"); // notice, this can actually be any field name that has the attribute, either change this method or maintain the naming convention
        ...
        public static string ToNotation<T>(this T item, NotationConfig notationCfg) => encodeInner(item, 0,
            VerifyNoCyclicalReference ? new HashSet<object>() : null, notationCfg);

        public static T FromQn<T>(string qn, bool tryQuotes = false) =>
            (T)decodeInner(qn, typeof(T), NotationConfig.Qn, tryQuotes);

        public static T FromText<T>(string txt, NotationConfig notationCfg, bool tryQuotes = false) =>
            (T)decodeInner(txt, typeof(T), notationCfg, tryQuotes);

        public static object FromText(string txt, NotationConfig notationCfg, Type t, bool tryQuotes = false) =>
            decodeInner(txt, t, notationCfg, tryQuotes);
        ...
        /// <summary>
        ///     Parse a block from stream, allowing separate QN or JSON objects to be read when they arrive on the same network
        ///     stream
        /// </summary>
        public static string ReadBlock(TextReader r, NotationConfig notationCfg = null)
        {
            ...
            while (blockStack.Count > 0) {
                ...
                if (lastChr == notationCfg.Quote) {
                    ...
                    if (lastChr == ',' || lastChr == ';'
                                       || lastChr == notationCfg.CloseRecord || lastChr == notationCfg.CloseArray ||
                                       lastChr == notationCfg.CloseDictionary
        ...
        static Type getNullableUnderlyingType(this Type type) => Nullable.GetUnderlyingType(type);
            ...
            try {
                ...
                if (data is string dataStr)
                    return notationCfg.EncodeStringAsDoubleQuote
                        ? notationCfg.Quote +
                          dataStr.Replace($"{notationCfg.Quote}", $"{notationCfg.Quote}{notationCfg.Quote}") +
                          notationCfg.Quote
                ...
                if (t.IsEnum) {
                    switch (notationCfg.EnumEncoding) {
                        ...
                        case EnumEncodingOption.QuotedName:
                            return $"{notationCfg.Quote}{Enum.GetName(t, data)}{notationCfg.Quote}";
                        case EnumEncodingOption.Number:
                ...
                if (t == typeof(byte[])) {
                    if (notationCfg.AddClassName)
                        return "Convert.FromBase64String(" + notationCfg.Quote + Convert.ToBase64String((byte[])data) +
                               notationCfg.Quote + ")";
                ...
                if (data is IDictionary d && (t.HasElementType || t.GenericTypeArguments.Length == 2)) {
                    if (notationCfg.OpenDictionary.Length == 1 || notationCfg.OpenDictionary.IndexOf('{') < 0)
                        ...
                        sb.Append(string.Format(notationCfg.OpenDictionary,
                            t.GetGenericArguments()[0].GetFriendlyName(),
                    ...
                    foreach (var key in d.Keys) {
                        if (!firstDicV)
                            ...
                            firstDicV = false;
                        if (!string.IsNullOrWhiteSpace(notationCfg.DictionaryItemOpen))
                            sb.Append(notationCfg.DictionaryItemOpen);
                        Dictionary<string, object> keyFaf = null;
                        if (key != null && kt != typeof(string) && kt.IsClass && kt != key.GetType() &&
                            !notationCfg.AddNewKeywordToClassName)
                        ...
                        if (!string.IsNullOrWhiteSpace(notationCfg.DictionaryItemClose))
                            sb.Append(notationCfg.DictionaryItemClose);
                    }
                ...
                foreach (var fi in fields) {
                    ...
                    if (v != null && fi.FieldType != typeof(string) && fi.FieldType.IsClass &&
                        fi.FieldType != v.GetType() && !notationCfg.AddNewKeywordToClassName)
                ...
                foreach (var pi in props) {
                    ...
                    if (v != null && pi.PropertyType != typeof(string) && pi.PropertyType.IsClass &&
                        pi.PropertyType != v.GetType() && !notationCfg.AddNewKeywordToClassName)
                ...
                if (CyclicalVerificationParentOnly && cyclesTraceList != null && data != null &&
                    data.GetType().IsClass && cyclesTraceList.Contains(data))
        ...
        static object decodeInner(Slice encoded, Type t, NotationConfig notationCfg, bool tryQuotes = false,
            HashSet<XmlIncludeAttribute> xmlIncludes = null)
        {
            if (encoded == null || encoded == notationCfg.NullStr || encoded.IsNullOrWhiteSpace()) return null;
            if (t == null || t == typeof(object) || t == typeof(UnparsedItem))
                return new UnparsedItem { Config = notationCfg, RawText = encoded };
            if (t.IsInterface) {
            ...
            if (t.IsEnum)
                switch (notationCfg.EnumEncoding) {
                    ...
                    case EnumEncodingOption.TypeDotName:
                        return Enum.Parse(t, encoded.Substring(encoded.IndexOf('.') + 1));
                }
            ...
            if (t == typeof(DateTime)) {
                if (DateTime.TryParseExact(encoded.Trim('\"'), notationCfg.DateFormat ?? "g",
            ...
            if (typeof(IDictionary).IsAssignableFrom(t)) {
                ...
                while (helper.Current != closeDic) {
                    ...
                    if (notationCfg.AddClassName && char.IsLetter(helper.Current))
                        helper.SkipToAny(
                            $" {notationCfg.OpenRecord}{notationCfg.OpenArray}{notationCfg.OpenDictionary}");
                    if (helper.Current == notationCfg.OpenRecord || helper.PeekPhrase(notationCfg.OpenDictionary) ||
                        helper.PeekPhrase(notationCfg.OpenArray))
                    ...
                    var key = decodeInner(keyPart, keyType, notationCfg, true, xmlIncludes);
                    helper.SkipOne(); // :
                    helper.SkipWhiteSpaces();
                    string vPart;
                    if (notationCfg.AddNewKeywordToClassName && helper.PeekPhrase("new ")) helper.SkipPhrase("new ");
                    ...
                    vPart = vPart.Trim(' ', '\t', '\r', '\n');
                    var v = decodeInner(vPart, valueType, notationCfg, true, xmlIncludes);
                    dic.Add(key, v);
                    helper.SkipWhiteSpaces();
            ...
            if (notationCfg.AddClassName && char.IsLetter(helper.Current))
                helper.SkipToAny($" {notationCfg.OpenRecord}{notationCfg.OpenArray}{notationCfg.OpenDictionary}");
            if (encoded.StartsWith(notationCfg.OpenRecord)) {
                ...
                while (helper.Current != notationCfg.CloseRecord) {
                    ...
                    helper.SkipWhiteSpaces();
                    if (fieldName == "_type") {
                        // _type must be the first field of the record otherwise all prior fields will be discarded
            ...
            if (encoded.StartsWith(notationCfg.OpenArray)) {
                ...
                while (helper.Current != notationCfg.CloseArray) {
                    ...
                    if (helper.Current == notationCfg.Quote) {
                        ...
                        if (notationCfg.NullStr.Length > 0 && notationCfg.NullStr[0] == notationCfg.Quote &&
                            (str == notationCfg.NullStr || str == notationCfg.NullStr.Trim(notationCfg.Quote))) {
                            list.Add(null);
                        } else if (elT == typeof(object) || elT == typeof(UnparsedItem)) {
                            list.Add(new UnparsedItem {
                                RawText = helper.AllText.Substring(strStartIdx, strEndIdx - strStartIdx),
                                Config = notationCfg
                            });
                        ...
                        var item = decodeInner(elPart, elT, notationCfg, true, xmlIncludes);
                        list.Add(item);
                    }
        ...
        public static string Escape(this string input, bool addQuotes = true, char quoteChar = '\"',
            bool excludeNonQuote = true)
        {
            ...
            string tab = "   ")
        {
            ...
            for (int i = 0; i < data.Length; i++) {
                ...
                if (char.IsWhiteSpace(c) ||
                    whites.IndexOf(c) >= 0) // unless char-space-char, this would be an unwanted behavior
                ...
                if (openerIdx < 0 && closerIdx < 0) continue;
        ...
        public static string GetFriendlyName(this Type type, Dictionary<Type, string> translations)
        {
            ...
            if (type.IsGenericType)
                return type.Name.Split('`')[0] + "<" + string.Join(", ",
                    type.GetGenericArguments().Select(GetFriendlyName).ToArray()) + ">";
            return type.Name;
        }
    ...
    public class Slice
    {
        ...
        public int Length
        {
            get => LengthOfSubstring < 0
                ? OriginalString.Length - StartIndex
                : Math.Min(LengthOfSubstring, OriginalString?.Length - StartIndex ?? 0);
    ...
    public class UnparsedItem
    {
        ...
        public bool IsClass
        {
            get
            {
                ...
                if (c == Config.Quote && !Config.FieldNameInQuotes || Config.FieldNameInQuotes && c != Config.Quote)
                    return false;
                if (c == Config.Quote)
        ...
        public bool IsBool
        {
            get
            {
                if (Config.BooleanAsLowecase && Config.BooleanInQuotes)
                    return RawText == $"{Config.Quote}true{Config.Quote}" ||
                           RawText == $"{Config.Quote}false{Config.Quote}";
        ...
        public Dictionary<string, UnparsedItem> ParseClass() => _dic ?? (_dic =
            IsClass ? Parse<Dictionary<string, UnparsedItem>>() : new Dictionary<string, UnparsedItem>());
        ...
        public object ParsePrimitive()
        {
            ...
            if (RawText.FindChar(".", out _) == '.') return ParseFloat();
            return ParseInt();
        }
    ...
    internal class ParseHelper
    {
        ...
        public bool HasEndedOrSquigglyBrackenEnd =>
            CurrentIndex < 0 || CurrentIndex >= AllText.Length || AllText[CurrentIndex] == '}';
